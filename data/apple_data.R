...
lizzo = apple_itunes %>% filter(artist =="Lizzo")
...
xcode = apple_apps %>% filter(app_name == 'Xcode')

get_apple_chart_postion( id = xcode$app_id, "ca")

get_apple_rating_split(id = xcode$app_id, country = "ca")

app_reviews = get_apple_reviews( xcode$app_id, "ca", all_results = T, page_no = 1)
